<?php

return [
    'title' => 'Assistenten',
    'subtitle' => 'Verwalten Sie die Assistenten, die Röntgenbehandlungen in Ihrer Praxis durchführen.',
    'search_placeholder' => 'Nach Assistenten-Code oder Name suchen',
    'create' => 'Assistent anlegen',
    'create_heading' => 'Assistent anlegen',
    'edit_heading' => 'Assistent bearbeiten',
    'empty' => 'Keine Assistenten entsprechen Ihren Filtern.',
    'code_placeholder' => 'NURSE_01',
    'deactivate_confirm_title' => 'Assistent deaktivieren',
    'deactivate_confirm' => ':name deaktivieren? Er/Sie steht für neue Röntgeneinträge nicht mehr zur Verfügung.',
    'deactivate_confirm_generic' => 'Diesen Assistenten deaktivieren? Er/Sie steht für neue Röntgeneinträge nicht mehr zur Verfügung.',
    'commission_rates' => [
        'heading' => 'Assistenten-Provisionen',
        'description' => 'Weisen Sie pro Röntgenbehandlung einen Provisionsprozentsatz zu. Der Assistent erscheint erst im Tagesbericht-Editor, nachdem hier ein Satz hinzugefügt wurde.',
        'empty' => 'Noch keine Provisionssätze.',
        'treatment' => 'Behandlung',
        'rate' => 'Satz',
        'select_treatment' => 'Behandlung wählen',
        'commission_percent' => 'Provision %',
        'add_rate' => 'Satz hinzufügen',
        'placeholder' => '5.00',
    ],
];
