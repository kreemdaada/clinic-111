    ...
    <aside class="dr-sidebar card">
        <div>
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.5rem;">
                ...
                @if (auth()->user()->isAdmin())
                <button type="button" class="btn btn-secondary btn-sm" id="dr-add-doctor-open">{{ __('daily_reports.actions.add_doctor') }}</button>
                @endif
            </div>
...
$drUiLabels = [
    'saveEntry' => __('daily_reports.actions.save_entry'),
    'saveChanges' => __('common.actions.save_changes'),
    'edit' => __('common.actions.edit'),
    'delete' => __('common.actions.delete'),
    'deleteRowTitle' => __('common.confirm.delete'),
    'deleteRowMessage' => __('daily_reports.actions.delete_row_message'),
];
@endphp
...
{!! json_encode($drUiLabels, JSON_THROW_ON_ERROR | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) !!}
</script>
<script>
    (function() {
        const drUiLabels = JSON.parse(document.getElementById('dr-ui-labels').textContent);
        const {
            reportId,
            month,
            readOnly,
        ...
        function resetEmptyDayForm() {
            ...
            saveBtn.textContent = drUiLabels.saveEntry;
        ...
        async function startEditRow(row) {
            ...
            saveBtn.textContent = drUiLabels.saveChanges;
        ...
        async function loadRows() {
            ...
            <div class="dr-row-card" data-row-id="${r.id}">
                ...
                <div class="dr-row-actions">
                    ${readOnly ? '' : `<button type="button" class="btn btn-primary btn-sm" data-edit-row="${r.id}">${drUiLabels.edit}</button>`}
                    ${readOnly ? '' : `<button type="button" class="btn btn-secondary btn-sm" data-delete-row="${r.id}">${drUiLabels.delete}</button>`}
                </div>
            </div>
        `;
            }).join('');
                ...
                btn.addEventListener('click', async () => {
                    const confirmed = typeof window.clinicConfirm === 'function' ?
                        await window.clinicConfirm({
                            title: drUiLabels.deleteRowTitle,
                            message: drUiLabels.deleteRowMessage,
                            okText: drUiLabels.delete,
                            danger: true,
                        }) :
                        true;
                    if (!confirmed) {
